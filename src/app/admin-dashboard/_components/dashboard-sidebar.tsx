  ...
  return (
    <div className="w-[300px] max-w-[354px] h-screen bg-white shadow-[4px_0px_10px_0px_rgba(0,0,0,0.25)] pl-7 sticky top-0 z-50 -mt-[86px] flex flex-col">
        ...
        <div className="mt-auto pb-10">
            ...
            <Button onClick={() => setIsOpen(true)}  variant="ghost" className="w-full flex items-center justify-start gap-4 text-lg font-medium text-black leading-[120%] shadow-none border-none "><LogOut /> Log out</Button>
        </div>

        {/* log out modal  */}
        {
          isOpen && (
            <LogoutModal isOpen={isOpen} onClose={() => setIsOpen(false)} onConfirm={() => signOut({callbackUrl: "/"})} />)
        }
    </div>
  );
};
